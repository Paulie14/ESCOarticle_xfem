...
\section{Appriori adaptive quadrature}

Let us assume just one well of radius $R$ situated at origin. Worst term to integrate is
\begin{equation}
    \label{eq:term-of-interest}
    f(r)=(\nabla \log r )^2 \approx r^{-2}
\end{equation}

independently of the particular variant of the XFEM method. Consider a square $S$ with side $h$ and 
let us denote $r_{min}$ and $r_{max}$ the minimum and maximum distance to the origin respectively.
We shall estimate error of the 2D tensor product Guass quadrature rule of order $n$ on the square $S$  
by the error of 1D quadrature of the same order on $(r_{min}, r_{min}+h)$:
\[
  E^n(S) \le h E^n((r_{min}, r_{min}+h))
\]
this is conservative estimate, which is natural for elements near main $x$ and $y$ axis, where the error is maximal.

To guarantee prescribed tolerance $\epsilon$, want to control the error density:
\begin{equation}
...
\]
then the convergence criterium folowing from \eqref{eq:error-density} reads:
\[
  h^{-1}E_n = \alpha_n \left( \frac{h}{r_{min}} \right)^{2n} \frac{1}{r_{min}^2} \le \epsilon, 
  \qquad \alpha_n = (2n+2)\left( \frac{(n!)^2}{(2n)!} \right)^2
\]

Derived criterium holds only on squares, where integrated function is smooth.
This is not the case for squares intersecting the boundary of well, $r_{min} \le R \le r_{max}$, where we integrate 
...
integral is
\[
    E(H) = c_e H^{p_e}, \quad \text{with } c_e=0.08,\ p_e=1.5.
\]
...
\]
TODO: modify test of integration on unit disk for the function $1/r^2$.


Based on presented analysis, we propose following rules for adaptive quadrature:
\begin{enumerate}
 \item If $r_{max} < R$ the sqare quadrature is zero.
...
original function (i.e. $f(r)=0$ for $r\le R$).
 ...
 \]
 use at least same order as necessary for FEM.
\end{enumerate}
