...
When solving the \probref{def:test_case_2}, graph in \fig{fig:convergence_sin} we put the convergence of the
standard FEM on the problem with the well omitted as a reference in the graph (finding only the regular part $u_r$), 
see the 'FEM (ur)' with optimal convergence rate 2.0. The closer the PUM methods are to 'FEM (ur)' the better 
...
%   \vspace{0pt}
  ...
  \probref{def:test_case_2}. The 'FEM (ur)'
