...
In order to improve the approximation of a singular behaviour of the pressure near the wells, the standard finite element space is enriched with
the cut-off fundamental solution to the Laplace problem. The optimal order of 
convergence in terms of $L^2$ norm of the error is demonstrated for some of the compared methods.
...
which is payed of by significantly larger number of degrees of freedom. One possible alternative is the usage of 
...
In contrast to the classical finite element method, the XFEM have to employ an adaptive integration scheme 
for asembly of the linear system on the enriched elements
in order to keep quadrature error small enough.
We derive an estimate for the quadrature error and propose a robust adaptive strategy based on the apriori knowledge 
...
The implementation\footnote{\url{https://github.com/Paulie14/xfem_project}} of the compared methods is done in C++ language 
...
however we add an artificial volume source term. This allows us to better study impact of 
the prescribed source on the solution which is better suited to the numerical experimants we are focused on. 

The aquifers are then connected only through the wells 
which acts as sources or sinks in the domain of each aquifer. The pressure in the aquifers is further governed 
...
$\vc{n}$ is the unit outer normal vector of the interior boundary (i.e. pointing to the center of wells),
...
\begin{itemize}
  ...
  \item the well edge geometry, since we have to integrate only outsid of the well.
\end{itemize}
...
least the ramp function must be used in order to obtain optimal convergence. \noteJB{Ale vyse piseme, ze XFEM konverguje stejne dobre i bez rampy.} The shifted XFEM and 
