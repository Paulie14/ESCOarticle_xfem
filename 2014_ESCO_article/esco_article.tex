...
$\vc{n}$ is unit outer normal vector on the interior boundary and
$q^m_w$ is the density of the flow from the well to the aquifer over the interior boundary $\partial B^m_w$. 
The total flow $Q^m_w$ from the well $w$ to aquitard $m$ is given 


Presuming the aquitards to be fully impermeable, the communication between aquifers is possible only through 
the wells. We can prescribe the flow balance equation 
\begin{eqnarray}
