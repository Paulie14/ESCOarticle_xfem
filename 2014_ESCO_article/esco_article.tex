...
We suggest additional criterion for subelements refinement which takes into account a subelement diameter 
...
where $h$ is the diameter of the subelement and $r_{min}$ is the minimal distance between a vertex of 
the subelement and the well center. $C_R$ is a scaling constant, equal 0.5 by default, through which we can 
control the significance of the criterion. If satisfied, the subelement is not refined anymore.
...
Eventually, we do 10 levels of improved adaptive refinement with the following rules ($r_{max}$ is the maximal
distance of a vertex of the subelement and the well center):
\begin{enumerate}
 \item If $r_{max} < \rho$, the square is not refined and the quadrature is zero.
 \item If $r_{min} < \rho < r_{max}$, the square is refined, $3\times3$ quadrature is set. If any of the 
 quadrature points lie in the well circle then they are removed.
 \item If $r_{min} > \rho$, we test the suggested criterion $h \leq C_R r_{min}$. If it is satisfied, the 
 $3\times3$ quadrature is set, else the subelement is further refined.
\end{enumerate}
...
\]
where $S'$ is square with side $H=h/\rho$ and $\vc {r'} = \vc{r}/\rho$. Empirically determined error of the midpoint rule for later 
integral is
\[
...
quadrature on the coarser level. As for the coefficient, these can be read from the graph -- for the one point
...
\]
Using standard error estimate for elliptic PDE (see e.g. \cite{ciarlet_basic_1991}), we get
\begin{equation}
...
dependence of global $L^2$ error on the enrichment radius in agreement with \eqref{eq:singular_approx_error}.
...
\bottomrule
\end{tabular}
\caption{Numerical validation of the estimate \eqref{eq:grad_estimate_on_square} in 2D case. The minimal and maximal 
values of \eqref{eqn:log_h1_estimate_ratio} are collected in the table for different refinements.
\notePE{DONE: Seems that there is still a coefficient close to $\sqrt{2}$.}}
\label{tab:log_h1_estimate}
\end{center}
\end{table}

Further we can see the error in $H^1$ norm over the elements in the \fig{fig:log_estimate_a}. We can 
compare it with the ratio in the \fig{fig:log_estimate_b} and see that the ratio is closest to 1.0 
...
We demonstrate the behavior of the model depending on the enrichment radius choice on our test case with 
...
radii. This trend is more obvious from the graph in \fig{fig:radius_conv_2} which shows the dependence of the
