...
The paper is organised as follows. The model and its weak formulation are introduced in Section \ref{sec:model}.
In section \ref{sec:discretization},  discretization and details of several PU methods are presented.

After that we investigate the problem of integration in the enrichment area and suggest some 
...
\begin{multline} \label{eqn:weak_form}
  ...
  \sum \limits_{m=1}^M \sum\limits_{w\in\mathcal{W}} c^{M+1}_w\left(H^M_w-H^{M+1}_w\right) \quad \forall m=1,\ldots,M.
\end{multline}
% \noteJB{Consider sum over aquifers to get square term from communication on wells.
% Boundary conditions on wells?}
...
%   \vspace{0pt}
  ...
  \probref{def:test_case_1}. The error is computed in $L^2$ norm.}
...
%   \vspace{0pt}
  ...
  \probref{def:test_case_2} in $L^2$ norm. The 'FEM (ur)'
