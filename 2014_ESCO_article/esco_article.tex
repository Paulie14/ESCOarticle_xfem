...
Let us assume, that the solution can be spitted to the singular part $u_s(\vc x)= \log |\vc x|$ and the regular part $u_r=u-u_s$.
...
by small level of quadrature refinement, by large contant for h/rho criterion (tried h/rho<=0.1). What should 
we blame except me?}
...
The Jacobi preconditioner was used each time. 
...
\end{center}
\end{table}
\noteJB{What is the min value on the last row?}
%
...
\end{equation}
on every element $T$ of the sequence of refined meshes using a $5\times5$ Gaussian quadrature for estimation of $H^1$ norm. Table 
\ref{tab:log_h1_estimate} reports the minimum and the maximum value of the ratio over all elements of every mesh.
The minimum value are close to 1 independently of $h$ so our estimate is tight. Moreover, the minimum value is attained on the majority of
...
for the linear elliptic problems (c.f. \cite{ciarlet_basic_1991}). According to the estimates \eqref{eq:std_err_estimate}
...
resolved by unenriched FEM displaying $O(h^2)$ convergence. As predicted the total error gets closer to the error of the 
regular part of the solution as the enrichment radius grows, but for certain critical $R$. 
...
The convergence graph in  shows how the error depends on the element size for different 
