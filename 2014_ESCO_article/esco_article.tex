...
which up to our best knowledge, is the first work using XFEM for the well problems.
...
Let $\Theta^m\subset \R^2$ be the domain of the $m$-th aquifer, $m=1,\ldots M$, and $z_m$ be its vertical coordinate.
\noteJB{are $z_m$ necessary?}
...
Actual computational domain of the aquifer $m$ is $\Omega^m = \Theta^m\setminus B^m$. The boundary $\partial\Omega^m$ of 
...
Poisson equation for the pressure head in the $m$-th aquifer:
\begin{equation} \label{eqn:poisson}
\nabla\cdot(-\mathbf{T}^m\nabla h^m) = f^m \qquad \textrm{on } \Omega^m\subset\R^2,\; \forall m=1,\dots,M, \\
\end{equation}
which has to be completed with boundary conditions
\begin{align}
...
$\vc{n}$ is unit outer normal vector on the interior boundary,
...
\]
satisfies simple balance equation on the well
\begin{equation}
    \label{eq:well_flow}
    Q_w^m = Q^m_{w,in} - Q^m_{w,out} = c^m_w\left( H^{m-1}_w-H^m_w \right) - c_w^{m+1}\left( H^m_w-H_w^{m+1}\right), 
\end{equation}

where $Q^m_{w,in}$ is flow from lower aquifer $m-1$, $Q^m_{w,out}$ is flow to the upper aquifer $m+1$, and 
$c^m_w\, [\textrm{m}^2\textrm{s}^{-1}]$ is the permeability of the well $w$ in the segment above the aquifer $m$.
In \eqref{eq:well_flow}, we assume Darcy flow in the well for the simplicity. The bottom of the well $w$ is impermeable,
we set $c^0_w=0$, $H^0_w=H^1_w$, and on the top we prescribe given pressure $H^{M+1}_w$.
