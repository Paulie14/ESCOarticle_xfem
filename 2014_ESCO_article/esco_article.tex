...
with the base center $\vc{x}_w$, radius $r_w$ and height $|z_1-z_M|$.  We further denote 
...
In contrast to global enrichment methods, XFEM and SGFEM apply the enrichment functions only locally.
The area of the enrichment in our case is a circle of radius $R$ around a well. $R$ will be further called
the enrichment radius.
...
$h$    & min & max \\
\midrule
$\rfrac{14}{8}$    & 1.38 & 10.0  \\ %& 0.7 & 5.3   \\
$\rfrac{14}{16}$   & 1.40 & 23.1  \\ %& 1.0 & 17.4  \\
$\rfrac{14}{32}$   & 1.41 & 48.7  \\ %& 1.5 & 51.8  \\
$\rfrac{14}{64}$   & 1.41 & 99.5  \\ %& 2.1 & 150   \\
$\rfrac{14}{128}$  & 1.41 & 201   \\ %& 3.0 & 427   \\
\bottomrule
\end{tabular}
...
Let us consider a 2D domain. The $H^1$ error on elements in distance $r$ is same as in the 1D case up to 
a constant close to 1 as we have seen above. Thus error on the band at distance $r$ is proportional to 
\begin{equation} \label{eqn:enr_choice_1}
 \|\log \vc x - u_h\|^2_{H^1(\Omega^P)} \approx \int_R^{diam \Omega} 2\pi r \frac{h^3}{12 r^4} \d r \le \frac{2\pi h^3}{12} 
\frac{1}{2R^2} \approx \frac{h^3}{4R^2}.
\end{equation}
Then, the optimal choice of $R$ for given $H^1$ tolerance $\epsilon$ follows directly from \eqref{eqn:enr_choice_1}
\begin{equation} \label{eqn:enr_choice_2}
  R = \frac{h^{3/2}}{4\epsilon}
\end{equation}
...
In order to measure convergence an analytic solution is needed. We suggest two following test cases which were
used in our numerical tests. We confined our tests to single aquifer problems for the purpose of this article 
but our implementation enables multi-aquifer systems as well.

\notePE{TODO: Move before the numerical tests.}
...
The input parameters for the numerical test of \probref{def:test_case_1} and \probref{def:test_case_2} 
are gather in the table \ref{tab:parameters}.
%
\begin{table}[!ht]
\begin{center}
\begin{tabular}{crr}
