...
%JB%This is not the case for squares intersecting the boundary of the well, $r_{min} \le \rho \le r_{max}$, where we integrate 
...
in next subsection \ref{sec:conditioning}.
...
\subsection{Conditioning of the system matrix} \label{sec:conditioning}
...
We observe the iteration counts needed by CG solver and gather them in the graph in \fig{fig:iterations}.
The Jacobi preconditioning is used each time and tolerance $10^{-9}$ is set. The number of iterations of the 
standard FEM is corresponding to the classic results as mentioned in the paragraph above. 
...
associated standard FEM system. This is satisfied in our case and we see from the graph in \fig{fig:iterations}
that the shifted XFEM has similarly good results. This area of the problem would need deeper investigation
and it will be probably a part of the following work.
